...
export const resetDailyBalances = async (): Promise<void> => {
    ...
    console.log(chalk.cyan(`[BalanceReset] Active subscriptions found: ${activeSubscriptions.length}`));
    ...
    if (expired.modifiedCount > 0) {
        console.log(chalk.yellow(`[BalanceReset] Expired ${expired.modifiedCount} subscriptions`));
    }
    ...
    cron.schedule(
        ...
        async () => {
            try {
                ...
                console.log(chalk.green(`[BalanceReset] Completed at ${new Date().toISOString()}`));
            } catch (err) {
