...
//TODO: get all notifications user and admin
...
const markAsRead = async (notificationId: string, userId: string) => {
  const notification = await NotificationModel.findOne({ _id: notificationId, receiverId: userId }); //TODO: check if notification belongs own
...
const deleteNotification = async (notificationId: string, userId: string) => {
  const notification = await NotificationModel.findOneAndDelete({ _id: notificationId, receiverId: userId }); //TODO: check if notification belongs own
  if (!notification) throw new CustomError(404, "Notification not found");
  await notification.save();
}
